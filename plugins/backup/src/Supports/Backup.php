...
class Backup
{
    ...
    {
        $backupFolder = $this->createFolder(base_path('backup'));
        $now = Carbon::now()->format('Y-m-d-h-i-s');
        $this->folder = $this->createFolder($backupFolder . DIRECTORY_SEPARATOR . $now);

        $file = base_path('backup/backup.json');
        $data = [];
    ...
    public function deleteFolderBackup($path)
    {
        ...
        if (empty($this->file->directories(base_path('backup')))) {
            $this->file->deleteDirectory(base_path('backup'));
        }
