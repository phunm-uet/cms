...
class AnalyticsController extends Controller
{
        ...
        try {
            ...
            if ($dimensions === 'hour') {
                foreach ($answer->rows as $dateRow) {
                    $visitorData[] = ['axis' => (int)$dateRow[0] . 'h', 'visitors' => $dateRow[1], 'pageViews' => $dateRow[2]];
                }
            } else {
                foreach ($answer->rows as $dateRow) {
                    $visitorData[] = ['axis' => Carbon::parse($dateRow[0])->toDateString(), 'visitors' => $dateRow[1], 'pageViews' => $dateRow[2]];
                }
            }
            ...
            return ['error' => false, 'data' => view('analytics::widgets.general.general', compact('stats', 'country_stats', 'total'))->render()];
    ...
    public function getTopVisitPages()
    {
        ...
        try {
            ...
            return ['error' => false, 'data' => view('analytics::widgets.page.page', compact('pages'))->render()];
        } catch (InvalidConfiguration $ex) {
            return ['error' => true, 'message' => $ex->getMessage()];
        }
    }
    ...
    public function getTopBrowser()
    {
        ...
        try {
            ...
            return ['error' => false, 'data' => view('analytics::widgets.browser.browser', compact('browsers'))->render()];
        } catch (InvalidConfiguration $ex) {
            return ['error' => true, 'message' => $ex->getMessage()];
        }
    }
    ...
    public function getTopReferrer()
    {
        ...
        try {
            ...
            return ['error' => false, 'data' => view('analytics::widgets.referrer.referrer', compact('referrers'))->render()];
        } catch (InvalidConfiguration $ex) {
            return ['error' => true, 'message' => $ex->getMessage()];
        }
    }
