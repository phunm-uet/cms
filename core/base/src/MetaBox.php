...
class MetaBox
{
    ...
    {
        try {
            $field_meta = $this->metaBoxRepository->getFirstBy(['content_id' => $content_id, 'meta_key' => $key, 'reference' => $reference]);
            if (!$field_meta) {
     ...
     * @author Sang Nguyen
     */
    public function getMetaData($content_id, $key, $reference, $single = false)
    {
        $field =  $this->getMeta($content_id, $key, $reference);
        if (!$field) {
            return $single ? '' : [];
        }
     ...
     * @author Sang Nguyen
     */
    public function getMeta($content_id, $key, $reference)
    {
        return $this->metaBoxRepository->getFirstBy(['content_id' => $content_id, 'meta_key' => $key, 'reference' => $reference]);
    }
    /**
     * @param $content_id
     * @param $key
    ...
    {
        return $this->metaBoxRepository->getFirstBy(['content_id' => $content_id, 'meta_key' => $key, 'reference' => $reference])->delete();
    }
}
