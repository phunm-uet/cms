...
class Theme implements ThemeContract
{
    ...
    )
    {
        ...
        SeoHelper::setTitle(setting('seo_title'), setting('site_title'))
            ->setDescription(setting('seo_description'))
            ->setKeywords(setting('seo_keywords'))
            ->meta()->setGoogle(setting('google_analytics'))
            ->addMeta('google', setting('google_site_verification'));
    }
    ...
    public function checkViewExists($view, $return = false)
    {
        ...
        if (!$this->view->exists($view)) {
            if (app()->environment() == 'local') {
                dd('Theme is not support this view, please file "/public/themes/' . str_replace($this->getThemeNamespace(), $this->getThemeName(), str_replace('::', '/', str_replace('.', '/', $view))) . '.blade.php" to render this page!');
            } else {
                abort(404);
            }
        }
