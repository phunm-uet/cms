                    ...
                    <div class="post-group__left">
                        <article class="post post__inside post__inside--feature">
                            <div class="post__thumbnail">
                                <img src="{{ url($feature_item->image) }}" alt="{{ $feature_item->name }}"><a href="{{ route('public.single.detail', $feature_item->slug) }}" class="post__overlay"></a>
                            </div>
                            <header class="post__header">
                                <h3 class="post__title"><a href="{{ route('public.single.detail', $feature_item->slug) }}">{{ $feature_item->name }}</a></h3>
                                <div class="post__meta"><span class="post-category"><i class="ion-cube"></i>
                                        @if (!$feature_item->categories->isEmpty())<a href="{{ route('public.single.detail', $feature_item->categories->first()->slug) }}">{{ $feature_item->categories->first()->name }}</a>@endif
                                        </span>
                    ...
                    @else
                        <div class="post-group__item">
                            <article class="post post__inside post__inside--feature post__inside--feature-small">
                                <div class="post__thumbnail"><img src="{{ url($feature_item->image) }}" alt="{{ $feature_item->name }}"><a href="{{ route('public.single.detail', $feature_item->slug) }}" class="post__overlay"></a></div>
                                <header class="post__header">
                                    <h3 class="post__title"><a href="{{ route('public.single.detail', $feature_item->slug) }}">{{ $feature_item->name }}</a></h3>
                                </header>
                            </article>
                                    ...
                                    @if ($loop->first)
                                        <div class="col-md-6 col-sm-6 col-xs-12">
                                            <article class="post post__vertical post__vertical--single">
                                                <div class="post__thumbnail">
                                                    <img src="{{ url($post->image) }}" alt="{{ $post->name }}"><a href="{{ route('public.single.detail', $post->slug) }}" class="post__overlay"></a>
                                                </div>
                                                <div class="post__content-wrap">
                                                    <header class="post__header">
                                                        <h3 class="post__title"><a href="{{ route('public.single.detail', $post->slug) }}">{{ $post->name }}</a></h3>
                                                    ...
                                                    <div class="post__footer"><a href="{{ route('public.single.detail', $post->slug) }}" class="post__readmore">{{ __('Read more') }}</a></div>
                                                </div>
                                            </article>
                                        </div>
                                        <div class="col-md-6 col-sm-6 col-xs-12">
